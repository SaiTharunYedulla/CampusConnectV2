...
interface TeacherDetail {
  ...
  bio?: string; profilePhoto?: string; designation?: string; employeeId?: string; expertise?: string;
  user: {
  ...
  stats?: { totalReviews: number; approved: number; rejected: number; revisionRequested: number };
}
...
export default function AdminTeacherDetailPage() {
  ...
  useEffect(() => {
    Promise.all([
      api.get(`/admin/teachers/${id}`).then(r => r.data.data),
      api.get('/ref/departments').then(r => r.data.data ?? []).catch(() => []),
    ])
      .then(([t, depts]) => {
        setTeacher(t);
        setDepartments(depts);
        if (t) setSelectedDept(t.department?.id ?? '');
      })
      .catch(() => router.push('/admin/teachers'))
      .finally(() => setLoading(false));
  }, [id, router]);

  const handleStatusChange = (to: UserStatus) => {
    if (to === 'REJECTED' || to === 'SUSPENDED') { setPendingTo(to); setShowReason(true); return; }
    doUpdate(to, '');
  };

  const doUpdate = async (to: UserStatus, r: string) => {
    setUpdating(true); setShowReason(false);
    try {
      await api.patch(`/admin/teachers/${id}/status`, { status: to, reason: r || undefined });
      setTeacher(prev => prev ? { ...prev, user: { ...prev.user, status: to } } : prev);
    } catch { /* ignore */ }
  ...
  const transitions = STATUS_TRANSITIONS[teacher.user.status] ?? [];

  return (
    <div className="animate-fade-in" style={{ maxWidth: '760px' }}>
      <PageHeader
        title={`${teacher.firstName} ${teacher.lastName}`}
        breadcrumbs={[{ label: 'Teachers', href: '/admin/teachers' }, { label: teacher.firstName }]}
      />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '1.25rem' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          <div className="cc-card" style={{ padding: '1.5rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.25rem' }}>
              <div style={{ width: '4rem', height: '4rem', borderRadius: '50%', background: '#ECFDF5', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem', fontWeight: 700, color: '#059669', overflow: 'hidden', border: '2px solid var(--border)' }}>
                {teacher.profilePhoto ? <img src={teacher.profilePhoto} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : teacher.firstName[0]?.toUpperCase()}
              </div>
              <div>
                <div style={{ fontWeight: 600, fontSize: '1.0625rem' }}>{teacher.firstName} {teacher.lastName}</div>
                <div style={{ color: 'var(--text-muted)', fontSize: '0.8125rem' }}>@{teacher.username}</div>
                {teacher.designation && <div style={{ color: '#059669', fontSize: '0.8125rem', fontWeight: 500, marginTop: '0.2rem' }}>{teacher.designation}</div>}
                <div style={{ marginTop: '0.375rem' }}><StatusBadge status={teacher.user.status} type="user" /></div>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.875rem' }}>
              {[
                ...
                { label: 'Expertise',  value: teacher.expertise ?? '—' },
                { label: 'Joined',     value: new Date(teacher.user.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' }) },
              ].map(({ label, value }) => (
                <div key={label}>
            ...
            {teacher.stats && (
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.625rem' }}>
                  {[['Total', teacher.stats.totalReviews, 'var(--primary)'], ['Approved', teacher.stats.approved, 'var(--success)'], ['Rejected', teacher.stats.rejected, 'var(--error)'], ['Revision', teacher.stats.revisionRequested, 'var(--warning)']].map(([label, value, color]) => (
                    <div key={String(label)} style={{ textAlign: 'center', padding: '0.625rem', background: 'var(--background)', borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
                      <div style={{ fontSize: '1.25rem', fontWeight: 700, color: String(color) }}>{value}</div>
          ...
          <div className="cc-card" style={{ padding: '1.5rem' }}>
            <h2 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '0.25rem' }}>Assign Branch / Department</h2>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.8125rem', marginBottom: '1.25rem' }}>Assign or update the teacher's department branch.</p>
            ...
            <form onSubmit={async (e) => {
              ...
              setProfileMsg('');
              try {
                const res = await api.patch(`/admin/teachers/${id}/profile`, {
                  departmentId: selectedDept || null,
                });
                setTeacher(prev => prev ? { ...prev, department: res.data.data?.department } : prev);
                setProfileMsg('Branch updated successfully!');
              } catch {
                setProfileMsg('Failed to update branch');
              } finally {
              ...
              <button type="submit" className="btn-primary" disabled={savingProfile} style={{ alignSelf: 'flex-start', padding: '0.5rem 1.25rem', fontSize: '0.875rem' }}>
                {savingProfile ? <><Loader2 size={14} className="animate-spin" />Saving…</> : 'Save Branch'}
              </button>
            </form>
          </div>
          ...
          <div className="cc-card" style={{ padding: '1.5rem' }}>
            ...
            {teacher.user.verificationDocuments.length === 0 ? (
              <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>No documents uploaded.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                {teacher.user.verificationDocuments.map(doc => (
                  <div key={doc.id} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem 1rem', border: '1px solid var(--border)', borderRadius: 'var(--radius)', background: 'var(--background)' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: '0.8125rem', fontWeight: 500 }}>{doc.fileName}</div>
        ...
        <div>
          <div className="cc-card" style={{ padding: '1.25rem', position: 'sticky', top: 'calc(var(--topbar-height) + 1.5rem)' }}>
            ...
            <div style={{ marginBottom: '1.25rem' }}><StatusBadge status={teacher.user.status} type="user" /></div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                ...
                {transitions.map(t => (
                  <button key={t.to} onClick={() => handleStatusChange(t.to)} disabled={updating}
                    style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.625rem 0.875rem', background: t.bg, border: '1px solid transparent', borderRadius: 'var(--radius)', color: t.color, fontWeight: 500, fontSize: '0.875rem', cursor: 'pointer', opacity: updating ? 0.6 : 1 }}>
